...
function createLayout() {
    ...
    spots.forEach(function (id, i) {
        ...
        button.style.width = stampSize + 'px';
        button.style.height = stampSize + 'px';
...
function updateCenter() {
    ...
    const count = spots.filter(function (id) {
...
function updateFinal() {
    ...
    const w = Math.min(96, size * 0.22);
    const h = Math.min(40, size * 0.1);
...
function checkCompletion(effect) {
    const allNormal = spots.every(function (id) {
        return localStorage.getItem(id) === 'done';
    });
